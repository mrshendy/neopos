...
{
    public $trx_date;
    public $type = 'in'; // in | out | transfer | direct_add
    ...
    public function refreshAllOnhand()
    {
        foreach (array_keys($this->rows) as $i) $this->refreshOnhand($i);
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            foreach ($this->rows as $r) {
                ...
                // ترحيل الرصيد إلى stock_balances (إن وجد)
                if (DB::getSchemaBuilder()->hasTable('stock_balances')) {
                    $qty = (float)$line->qty;
                    $uom = $line->uom ?: '';
                    if (in_array($this->type, ['in','direct_add'])) {
                        if ($this->warehouse_to_id) {
                            DB::statement("
                                INSERT INTO stock_balances (warehouse_id, product_id, uom, onhand, created_at, updated_at)
                                VALUES (?, ?, ?, ?, NOW(), NOW())
                                ON DUPLICATE KEY UPDATE onhand = onhand + VALUES(onhand), updated_at = NOW()
                            ", [$this->warehouse_to_id, $line->product_id, $uom, $qty]);
                        }
                        ...
                        if ($this->warehouse_from_id) {
                            DB::statement("
                                ...
                                VALUES (?, ?, ?, ?, NOW(), NOW())
                                ON DUPLICATE KEY UPDATE onhand = onhand - VALUES(onhand), updated_at = NOW()
                            ", [$this->warehouse_from_id, $line->product_id, $uom, $qty]);
                        }
