...
class Index extends Component
{
    ...
    protected $messages = [
        ...
        'transaction_sequences.pattern.required' => 'نمط الترقيم مطلوب',
    ];

    public function mount()
    {
        $neg = setting::firstOrCreate(
            ['key' => 'negative_stock_policy'],
            ['value' => ['policy' => 'block']]
        );

        $exp = setting::firstOrCreate(
            ['key' => 'expiry_alert_days'],
            ['value' => ['days' => 30]]
        );

        $seq = setting::firstOrCreate(
            ['key' => 'transaction_sequences'],
            ['value' => ['pattern' => 'INV-TRX-{YYYY}-{####}']]
        );

        $this->negative_stock_policy = $neg->value['policy'] ?? 'block';
        $this->expiry_alert_days     = (int)($exp->value['days'] ?? 30);
        $this->transaction_sequences = [
            'pattern' => $seq->value['pattern'] ?? 'INV-TRX-{YYYY}-{####}',
        ];
    }
    ...
    public function save()
    {
        ...
        // تنظيف بسيط للنمط (trim)
        $pattern = trim($this->transaction_sequences['pattern'] ?? '');

        setting::updateOrCreate(
            ['key' => 'negative_stock_policy'],
            ['value' => ['policy' => $this->negative_stock_policy]]
        );

        setting::updateOrCreate(
            ['key' => 'expiry_alert_days'],
            ['value' => ['days' => (int) $this->expiry_alert_days]]
        );
