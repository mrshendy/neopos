    ...
    <div class="col-12">
                    ...
                    <div class="card-header">
                       ...
                       {{-- Message --}}
                        ...
                        <div class="d-grid gap-2" >
                            <button data-bs-toggle="modal" data-bs-target="#signupModals" class="btn btn-primary waves-effect waves-light" type="button">{{ trans('settings_trans.add_new_currencies') }}</button>
                        </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="badge badge-label bg-success"><i class="mdi mdi-circle-medium"></i> {{ trans('settings_trans.default_true') }}</span>
                                         @endif 
                                         @if($currencies->status==0) 
                                            <span class="badge badge-label bg-warning"><i class="mdi mdi-circle-medium"></i> {{ trans('settings_trans.default_false') }}</span>
                                          @endif
                                    </td>
                                    <td>{{$currencies->created_at}}</td>
                                    <td>
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li><button data-bs-toggle="modal" data-bs-target="#signupModals{{$currencies->id}}" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</button></li>
                                                <li><button data-bs-toggle="modal" data-bs-target="#signupModal{{$currencies->id}}" class="dropdown-item edit-item-btn"> <i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</button></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
                                          <!-- edit -->
                                          <div id="signupModals{{$currencies->id}}" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;" >
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content border-0 overflow-hidden">
                                                    ...
                                                    <div class="modal-body">
                                                        <form action="{{ route('currencies.update','test')}}" method="post" >
                                                            {{ method_field('patch') }}
                                                            @csrf
                                                            <div class="form-group">
                                                                <label for="name_ar" class="form-label">{{ trans('settings_trans.name_ar') }}</label>
                                                                <input type="text" class="form-control" id="name_ar" name="name_ar"
                                                                       value="{{$currencies->getTranslation('name','ar')}}">
                                                                <input  class="form-control" id="id" name="id"
                                                                        value="{{$currencies->id}}" type="hidden">
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="name_en" class="form-label">{{ trans('settings_trans.name_en') }}</label>
                                                                <input type="text" class="form-control" id="name_en" name="name_en" value="{{$currencies->getTranslation('name','en')}}">
                                                            </div>
                                                            <div class="form-group">
                                                                ...
                                                                <select name="default" id="default" class="form-control" required>
                                                                    <option value="1" @if($currencies->default==1)selected @endif >{{ trans('settings_trans.default_true') }}</option>
                                                                    <option value="0" @if($currencies->default==0)selected @endif >{{ trans('settings_trans.default_false') }}</option>
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                ...
                                                                <select name="status" id="statusf" class="form-control" required>
                                                                    <option value="1" @if($currencies->status==1)selected @endif >{{ trans('settings_trans.default_true') }}</option>
                                                                    <option value="0" @if($currencies->status==0)selected @endif >{{ trans('settings_trans.default_false') }}</option>
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                        ...
                                        <div id="signupModal{{$currencies->id}}" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content border-0 overflow-hidden">
                                                    <div class="modal-header p-3">
                                                        <h4 class="card-title mb-0">{{ trans('settings_trans.massagedelete_currencies') }}</h4>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body text-center p-5">
                                                        ...
                                                        <div class="mt-4 pt-4">
                                                            ...
                                                            <p class="text-muted">   {{ trans('settings_trans.massagedelete_p') }}{{$currencies->name}}</p>
                                                            <!-- Toogle to second dialog -->
                                                            <form action="{{ route('currencies.destroy','test')}}" method="post">
                                                                {{ method_field('delete') }}
                                                                {{ csrf_field() }}
                                                                <input  class="form-control" id="id" name="id" value="{{$currencies->id}}" type="hidden">

                                                            <button class="btn btn-warning" data-bs-target="#secondmodal" data-bs-toggle="modal" data-bs-dismiss="modal">
                                                                {{ trans('settings_trans.massagedelete_currencies') }}
                                                            </button>
                                                           </form>
                                                        </div>
                                                    </div>
        ...
        <div id="signupModals" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 overflow-hidden">
                    <div class="modal-header p-3">
                        <h4 class="card-title mb-0">{{ trans('settings_trans.add_new_currencies') }}</h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
