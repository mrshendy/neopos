<div class="container-fluid px-3" x-data>
    ...
    <style>
        :root{--bg:#f7fafc;--card:#fff;--text:#0f172a;--muted:#64748b;--border:#e2e8f0;--radius:12px;--shadow:0 1px 3px rgba(0,0,0,.08);--shadow-md:0 6px 16px rgba(0,0,0,.08);--sky:#0ea5e9;--sky-600:#0284c7;--sky-300:#7dd3fc;--accent:#8b5cf6}
        *{box-sizing:border-box} body{background:var(--bg);color:var(--text)}
        .hero{position:relative;border-radius:16px;overflow:hidden;margin:10px 0 14px;border:1px solid var(--border);background:radial-gradient(800px 400px at 100% -20%,rgba(255,255,255,.35) 0%,transparent 70%),linear-gradient(135deg,var(--sky) 0%,var(--sky-600) 100%);color:#fff;box-shadow:var(--shadow-md)}
        ...
        .pos-grid{display:grid;grid-template-columns:1fr 420px;gap:14px}
        ...
        .chip:hover{background:#f8fafc;border-color:var(--sky-300)} .chip.active{background:var(--sky);color:#fff;border-color:var(--sky)}
        ...
        .product-thumb{height:88px;border-radius:10px;background:linear-gradient(135deg,#eef7ff,#e6f6ff);display:flex;align-items:center;justify-content:center;color:var(--sky-600);font-size:1.5rem;margin-bottom:8px;overflow:hidden}
        ...
        .cart-thumb{width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#eef7ff,#e6f6ff);display:flex;align-items:center;justify-content:center;color:var(--sky-600);overflow:hidden}
...
<i class="mdi mdi-clipboard-list-outline"></i>        <!-- نسخة أنيقة وأقرب للـ fa -->
            ...
            <div class="hero-actions">
                <div class="stat"><i class="fa-regular fa-calendar"></i><span class="small">{{ __('pos.today') }}: {{ $pos_date }}</span></div>
                <a class="btn-ghost" href="{{ route('pos.index') }}"><i class="fa-solid fa-list"></i> {{ __('pos.sales_list') }}</a>
                <button class="btn-ghost" type="button" data-bs-toggle="modal" data-bs-target="#orderPreviewModal" {{ empty($rows)||!$warehouse_id ? 'disabled' : '' }}><i class="fa-regular fa-eye"></i> {{ __('pos.preview_order') }}</button>
                <button class="btn-ghost" type="button" onclick="printOrder()" {{ empty($rows)||!$warehouse_id ? 'disabled' : '' }}><i class="fa-solid fa-print"></i> {{ __('pos.print') }}</button>
            </div>
        </div>
    </div>
        ...
        <div class="row g-2 align-items-center">
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" placeholder="{{ __('pos.barcode_search_ph') }}" wire:model.defer="barcode" wire:keydown.enter.prevent="addByBarcode">
                </div>
            ...
            <div class="col-md-3">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" placeholder="{{ __('pos.product_search_ph') }}" wire:model.debounce.350ms="filterProductText">
                </div>
            </div>
            <div class="col-md-2">
                <input class="form-control" list="dl-customers" placeholder="{{ __('pos.search_customer_ph') }}" wire:model.lazy="customerSearch" wire:change="selectCustomerBySearch">
            ...
            <div class="col-md-2">
                <input class="form-control" list="dl-warehouses" placeholder="{{ __('pos.search_warehouse_ph') }}" wire:model.lazy="warehouseSearch" wire:change="selectWarehouseBySearch">
            ...
            <div class="col-md-2">
                <input class="form-control" list="dl-cats" placeholder="{{ __('pos.search_category_ph') }}" wire:model.lazy="categorySearch" wire:change="selectCategoryBySearch">
                <datalist id="dl-cats">
                ...
                <div class="products-grid">
                    ...
                    @forelse($grid as $p)
                        @php
                            ...
                            $matrix = $p->units_map ?? ((is_string($p->units_matrix) && (str_starts_with(trim($p->units_matrix),'{')||str_starts_with(trim($p->units_matrix),'['))) ? (json_decode($p->units_matrix,true) ?: []) : []);
                            $defKey = $p->sale_unit_key ?: (array_key_first($matrix) ?? null);
                            $minP   = $defKey && isset($matrix[$defKey]['price']) ? (float)$matrix[$defKey]['price'] : (float)($p->min_price ?? 0);

                            $img = $p->thumb_url ?? ($p->image_path ? asset('attachments/'.ltrim($p->image_path,'/')) : asset('assets/images/file.png'));
                        @endphp
                        @if($passCat && $match)
                            <div class="product-card" wire:click="addProductToCart({{ $p->id }})" title="{{ __('pos.add_to_cart') }}">
                                <div class="product-thumb">
                                    @if($img)
                                        <img src="{{ $img }}" alt="{{ $pname }}" style="width:100%;height:100%;object-fit:cover;border-radius:10px" onerror="this.style.display='none'">
                                    @else
        ...
        <aside class="panel">
            <div class="panel-body">
                ...
                <div class="p-2 border rounded-3 mb-3">
                    <div class="row g-2">
                        <div class="col-6">
                            <label class="small mb-1">{{ __('pos.payment_method') }}</label>
                            <select class="form-select form-select-sm" wire:model="payment_method">
                                <option value="cash">{{ __('pos.pm_cash') }}</option>
                                <option value="card">{{ __('pos.pm_card') }} (Visa)</option>
                                <option value="instapay">{{ __('pos.pm_instapay') }}</option>
                                <option value="wallet">{{ __('pos.pm_wallet') }}</option>
                            </select>
                        </div>
                        <div class="col-6">
                            <label class="small mb-1">{{ __('pos.currency') }}</label>
                            <select class="form-select form-select-sm" wire:model="currency_id">
                        ...
                        <div class="col-12">
                            <label class="small mb-1">{{ __('pos.treasury') }}</label>
                            <select class="form-select form-select-sm" wire:model="finance_settings_id">
                                @foreach($finSettings as $fs)
                                    <option value="{{ $fs->id }}">{{ $fs->name }} — {{ __('pos.branch') }} #{{ $fs->branch_id }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="p-2 border rounded-3 mb-3">
                    <div class="row g-2 align-items-end">
                        <div class="col-8">
                            <label class="small mb-1">{{ __('pos.coupon_code') }}</label>
                            <input type="text" class="form-control form-control-sm" wire:model.defer="coupon_code" placeholder="ABC123">
                        </div>
                        <div class="col-4">
                            <button type="button" class="btn btn-outline w-100" wire:click="applyCoupon">{{ __('pos.apply') }}</button>
                        </div>
                        @if($coupon_applied)
                            <div class="col-12 small text-success"><i class="fa-solid fa-badge-check"></i> {{ __('pos.coupon_applied') }} — {{ $coupon_note }}</div>
                        @endif
                        @if($coupon_error)
                            <div class="col-12 small text-danger">{{ $coupon_error }}</div>
                        @endif
                    </div>
                </div>
                    ...
                    @forelse($rows as $i=>$r)
                        <div class="cart-item" wire:key="row-{{ $i }}">
                            <div class="cart-thumb">
                                ...
                                @if($rimg)
                                    <img src="{{ $rimg }}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:10px" onerror="this.style.display='none'">
                                @else
                            ...
                            <div class="flex-fill">
                                ...
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    <select class="form-select form-select-sm" style="width:auto" wire:model="rows.{{ $i }}.unit_id" wire:change="rowUnitChanged({{ $i }})">
                ...
                <div class="summary">
                    ...
                    @if($coupon_value>0)
                        <div class="summary-row"><span>{{ __('pos.coupon_discount') }}</span><span>-{{ number_format((float)$coupon_value,2) }}</span></div>
                    @endif
                    <div class="summary-row align-items-center">
                    ...
                    <div class="summary-row">
                        <span>{{ __('pos.grand_total') }}</span><strong>{{ number_format((float)$grand,2) }}</strong>
                    </div>

                    <div class="summary-row align-items-center">
                        <span>{{ __('pos.amount_paid') }}</span>
                        <input type="number" step="0.01" class="form-control form-control-sm" style="width:140px" wire:model.debounce.400ms="amount_paid">
                    </div>
                    <div class="summary-row">
                        <span>{{ __('pos.change_due') }}</span><strong>{{ number_format((float)$change_due,2) }}</strong>
                    </div>
                    ...
                    <div class="d-grid mt-2">
                        <button class="btn-outline" type="button" wire:click="clearCart"><i class="fa-solid fa-trash me-1"></i>{{ __('pos.clear_cart') }}</button>
                    </div>
                </div>
            </div>
        </aside>
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-between small">
                        <div>
                            ...
                            <div class="text-muted">{{ __('pos.warehouse') }}:
                                @php $w=$warehouses->firstWhere('id',$warehouse_id); $raw=$w->name ?? null; $wname=is_array($raw)?($raw[app()->getLocale()] ?? ($raw['ar'] ?? (reset($raw)??''))):$raw; @endphp
                                {{ $wname ?? '—' }}
                            </div>
                            <div class="text-muted">{{ __('pos.customer') }}:
                                @php $c=$customers->firstWhere('id',$customer_id); $raw=$c->name ?? null; $cname=is_array($raw)?($raw[app()->getLocale()] ?? ($raw['ar'] ?? (reset($raw)??''))):$raw; @endphp
                                {{ $cname ?? '—' }}
                            </div>
                        </div>
                        <div class="text-end">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered">
                            ...
                            @if($coupon_value>0)<tr><th colspan="4" class="text-end">{{ __('pos.coupon_discount') }}</th><th class="text-end">-{{ number_format((float)$coupon_value,2) }}</th></tr>@endif
