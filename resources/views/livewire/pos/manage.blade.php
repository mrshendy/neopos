    ...
    <style>
        ...
        * {
        ...
        .product-thumb {
            height: 88px;
            border-radius: 10px;
            background: linear-gradient(135deg, #eef7ff, #e6f6ff);
            display: flex;
            align-items: center;
            justify-content: center;
            color: var(--sky-600);
    ...
    <div class="hero">
        <div class="hero-inner">
            ...
            <div class="hero-actions">
                ...
                <a class="btn-ghost" href="{{ route('pos.index') }}"><i class="fa-solid fa-list"></i>
                    {{ __('pos.sales_list') }}</a>
                <button class="btn-ghost" type="button" data-bs-toggle="modal" data-bs-target="#orderPreviewModal"
                    {{ empty($rows) || !$warehouse_id ? 'disabled' : '' }}>
                    <i class="fa-regular fa-eye"></i> {{ __('pos.preview_order') }}
                </button>
                <button class="btn-ghost" type="button" onclick="printOrder()"
                    {{ empty($rows) || !$warehouse_id ? 'disabled' : '' }}>
    ...
    <form wire:submit.prevent="save" class="pos-grid">
        ...
        <section class="panel">
            <div class="panel-body">
                {{-- Category chips --}}
                <div class="chips">
                    <div class="chip {{ empty($activeCategoryId) ? 'active' : '' }}" wire:click="selectCategory(null)">
                        {{ __('pos.all') }}</div>
                    @foreach ($categories as $cat)
                        @php
                            $raw = $cat->name;
                ...
                <div class="products-grid">
                    ...
                    @forelse($grid as $p)
                        @php
                            ...
                            // مصفوفة الوحدات من الحقل units_matrix (لو ما عندك Accessor)
                            $matrix =
                                $p->units_map ??
                                (is_string($p->units_matrix) &&
                                (str_starts_with(trim($p->units_matrix), '{') ||
                                    str_starts_with(trim($p->units_matrix), '['))
                                    ? (json_decode($p->units_matrix, true) ?:
                                    [])
                                    : []);
                            $defKey = $p->sale_unit_key ?: array_key_first($matrix) ?? null;
                            $minP =
                                $defKey && isset($matrix[$defKey]['price'])
                                    ? (float) $matrix[$defKey]['price']
                                    : (float) ($p->min_price ?? 0);
            ...
            <div class="panel-body">
                <div class="row g-2 mb-3">
                    ...
                    <div class="col-6">
                        <input type="text" class="form-control form-control-sm"
                            placeholder="{{ __('pos.note') }}" wire:model.defer="notes_ar">
                    </div>
                </div>
                        ...
                        <div class="cart-item" wire:key="row-{{ $i }}">
                            ...
                            <div class="flex-fill">
                                ...
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    ...
                                    <span
                                        class="price-badge">{{ number_format((float) ($r['unit_price'] ?? 0), 2) }}</span>
                                </div>
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <div class="px-2" style="min-width:32px;text-align:center">
                                    {{ (float) ($r['qty'] ?? 0) }}</div>
                                <button type="button" class="qty-btn"
                                    wire:click="incQty({{ $i }})">+</button>
                                <button class="btn-outline btn btn-sm" type="button"
                                    wire:click="removeRow({{ $i }})" title="{{ __('pos.delete') }}">
                ...
                <div class="summary">
                    <div class="summary-row">
                        <span>{{ __('pos.subtotal') }}</span><span>{{ number_format((float) $subtotal, 2) }}</span>
                    </div>
                    <div class="summary-row align-items-center">
                        <span>{{ __('pos.discount') }}</span>
                        <input type="number" step="0.01" class="form-control form-control-sm"
                            style="width:120px" wire:model.lazy="discount">
                    </div>
                    <div class="summary-row align-items-center">
                        <span>{{ __('pos.tax') }}</span>
                        <input type="number" step="0.01" class="form-control form-control-sm"
                            style="width:120px" wire:model.lazy="tax">
                    </div>
                    ...
                    <div class="d-grid mt-3">
                        <button class="btn btn-primary" type="submit" wire:loading.attr="disabled">
                            ...
                            <i
                                class="fa-regular fa-floppy-disk me-1"></i>{{ $pos_id ? __('pos.update_btn') : __('pos.save_finish') }}
                        </button>
                    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content" id="printArea">
                <div class="modal-header">
                    <h6 class="modal-title"><i class="fa-solid fa-receipt me-1"></i>{{ __('pos.order_preview') }}
                    </h6>
                ...
                <div class="modal-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered">
                            ...
                            <tfoot>
                                <tr>
                                    <th colspan="4" class="text-end">{{ __('pos.subtotal') }}</th>
                                    <th class="text-end">{{ number_format((float) $subtotal, 2) }}</th>
                                </tr>
                                <tr>
                                    <th colspan="4" class="text-end">{{ __('pos.discount') }}</th>
                                    <th class="text-end">{{ number_format((float) $discount, 2) }}</th>
                                </tr>
                                <tr>
                                    <th colspan="4" class="text-end">{{ __('pos.tax') }}</th>
                                    <th class="text-end">{{ number_format((float) $tax, 2) }}</th>
                                </tr>
                                <tr>
                                    <th colspan="4" class="text-end">{{ __('pos.grand_total') }}</th>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" onclick="printOrder()"><i
                            class="fa-solid fa-print me-1"></i>{{ __('pos.print') }}</button>
                </div>
            </div>
        </div>
    </div>
