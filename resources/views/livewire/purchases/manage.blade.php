    ...
    <div class="card shadow-sm rounded-4 stylish-card mb-4">
        <div class="card-header bg-light fw-bold d-flex align-items-center justify-content-between">
            <div>
                ...
                {{ $purchase_id ? __('pos.purchase_title_edit') : __('pos.purchase_title') }}
                @if($purchase_id)
                    <span class="badge bg-light text-dark border ms-2">#{{ $purchase_id }}</span>
                @endif
            </div>
            <div class="toolbar">
                <a href="{{ route('purchases.index') }}" class="btn btn-outline-secondary btn-sm px-3">
        ...
        <div class="card-body">
            <form wire:submit.prevent="save" class="row g-3">
                ...
                <div class="col-lg-3">
                    ...
                    <div class="preview"><i class="mdi mdi-calendar"></i> <span>{{ $purchase_date ?: '—' }}</span></div>
                ...
                <div class="col-lg-3">
                    ...
                    <select class="form-select" wire:model="warehouse_id">
                        ...
                        @foreach($warehouses as $w)
                            ...
                            @endphp
                            <option value="{{ $w['id'] ?? $w->id }}">{{ $wname }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-lg-3">
                    ...
                    <select class="form-select" wire:model="supplier_id">
                        ...
                        @foreach($suppliers as $s)
                            ...
                            @endphp
                            <option value="{{ $s['id'] ?? $s->id }}">{{ $sname }}</option>
                        @endforeach
                    </select>
                    <div class="preview"><i class="mdi mdi-truck-outline"></i> <span class="badge badge-soft">{{ $supplier_id ?: '—' }}</span></div>
                                        ...
                                        <td>
                                            <select class="form-select"
                                                ...
                                                @foreach($products as $p)
                                                    @if((int)($p['category_id'] ?? $p->category_id) === (int)($r['category_id'] ?? 0))
                                                        @php
                                                            $pname = $p['name'] ?? $p->name ?? '';
                                                        ...
                                                        @endphp
                                                        <option value="{{ $p['id'] ?? $p->id }}">{{ $pname }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        ...
                                        <td>
                                            <select class="form-select" wire:model="rows.{{ $i }}.unit_id">
                                                ...
                                                @foreach($units as $u)
                                                    ...
                                                    @endphp
                                                    <option value="{{ $u['id'] ?? $u->id }}">{{ $uname }}</option>
                                                @endforeach
                                            </select>
                ...
                <div class="col-12 d-flex gap-2">
                    <button type="submit" class="btn btn-success rounded-pill px-4 shadow-sm" wire:loading.attr="disabled">
                        ...
                        {{ $purchase_id ? __('pos.update') : __('pos.save') }}
                    </button>
