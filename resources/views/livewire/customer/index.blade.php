<div>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h4 class="mb-0"><i class="mdi mdi-account-group-outline me-2"></i> {{ __('pos.title_customers_index') }}</h4>
        <a href="{{ route('customers.create') }}" class="btn btn-primary rounded-pill shadow-sm">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label fw-bold"><i class="mdi mdi-magnify me-1"></i>
                        {{ __('pos.ph_search_name') }}</label>
                ...
                <div class="col-md-2">
                    <label class="form-label fw-bold"><i class="mdi mdi-shield-key-outline me-1"></i>
                        {{ __('pos.th_code') }}</label>
                    <input type="text" class="form-control" wire:model.debounce.500ms="code"
                        placeholder="{{ __('pos.th_code') }}">
                    <small class="text-muted">{{ __('pos.preview') }}</small>
                ...
                <div class="col-md-2">
                    <label class="form-label fw-bold"><i class="mdi mdi-receipt-text-outline me-1"></i>
                        {{ __('pos.f_tax') }}</label>
                    <input type="text" class="form-control" wire:model.debounce.500ms="tax"
                ...
                <tbody>
                    @forelse($customers as $row)
                        ...
                        <tr>
                            ...
                            <td>{{ $name }}</td>
                            <td>{{ $row->phone }}</td>
                            <td>{{ $row->city ?? optional($row->cityRel)->name }}</td>
...
<script>
    function confirmDelete(id) {
        Swal.fire({
            title: 'تحذير',
            text: '⚠️ هل أنت متأكد أنك تريد حذف هذا العميل؟ لا يمكن التراجع بعد الحذف!',
            icon: 'warning',
