            ...
            <form wire:submit.prevent="update" class="row g-3">
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-identifier me-1 text-primary"></i> {{ __('pos.supplier_code') }}
                    </label>
                    <input type="text" class="form-control @error('code') is-invalid @enderror"
                        wire:model.defer="code" placeholder="{{ __('pos.ph_code') }}">
                    <small class="text-muted d-block">{{ __('pos.desc_code') }}</small>
                    @if (!empty($code))
                        <div class="preview-box mt-1 p-2 border rounded bg-light">{{ $code }}</div>
                    @endif
                    @error('code')
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-alphabetical me-1 text-primary"></i> {{ __('pos.name_ar') }}
                    </label>
                    <input type="text" class="form-control @error('name.ar') is-invalid @enderror"
                        wire:model.defer="name.ar" placeholder="{{ __('pos.ph_name_ar') }}">
                    <small class="text-muted d-block">{{ __('pos.desc_name_ar') }}</small>
                    @if (!empty($name['ar']))
                        <div class="preview-box mt-1 p-2 border rounded bg-light">{{ $name['ar'] }}</div>
                    @endif
                    @error('name.ar')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-alphabetical-variant me-1 text-primary"></i> {{ __('pos.name_en') }}
                    </label>
                    <input type="text" class="form-control @error('name.en') is-invalid @enderror"
                        wire:model.defer="name.en" placeholder="{{ __('pos.ph_name_en') }}">
                    <small class="text-muted d-block">{{ __('pos.desc_name_en') }}</small>
                    @if (!empty($name['en']))
                        <div class="preview-box mt-1 p-2 border rounded bg-light">{{ $name['en'] }}</div>
                    @endif
                    @error('name.en')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-briefcase me-1 text-primary"></i> {{ __('pos.commercial_register') }}
                    </label>
                    <input type="text" class="form-control" wire:model.defer="commercial_register"
                        placeholder="{{ __('pos.ph_cr') }}">
                    <small class="text-muted d-block">{{ __('pos.desc_cr') }}</small>
                    @if (!empty($commercial_register))
                        <div class="preview-box mt-1 p-2 border rounded bg-light">{{ $commercial_register }}</div>
                    @endif
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-receipt-text-outline me-1 text-primary"></i> {{ __('pos.tax_number') }}
                    </label>
                    <input type="text" class="form-control" wire:model.defer="tax_number"
                        placeholder="{{ __('pos.ph_tax') }}">
                    <small class="text-muted d-block">{{ __('pos.desc_tax') }}</small>
                    @if (!empty($tax_number))
                        <div class="preview-box mt-1 p-2 border rounded bg-light">{{ $tax_number }}</div>
                    @endif
                </div>
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-format-list-bulleted me-1 text-primary"></i> {{ __('pos.category') }}
                    </label>
                    <select class="form-select @error('supplier_category_id') is-invalid @enderror"
                        wire:model="supplier_category_id">
                ...
                <div class="col-md-4">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-cash-multiple me-1 text-primary"></i> {{ __('pos.payment_term') }}
                    </label>
                    <select class="form-select @error('payment_term_id') is-invalid @enderror"
                        wire:model="payment_term_id">
                        <option value="">{{ __('pos.ph_payment_term') }}</option>
                        @foreach ($terms as $t)
                            <option value="{{ $t->id }}">
                                {{ app()->getLocale() == 'ar' ? $t->getTranslation('name', 'ar') : $t->getTranslation('name', 'en') }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- الجغرافيا: دولة / محافظة / مدينة / منطقة --}}
                <div class="col-md-3">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-earth me-1 text-primary"></i> {{ __('pos.country') }}
                    </label>
                    <select class="form-select @error('country_id') is-invalid @enderror" wire:model="country_id">
                        ...
                        @foreach ($country as $c)
                            <option value="{{ $c->id }}">
                                {{ $c->name ?? ($c->getTranslation('name', 'ar') ?? $c->getTranslation('name', 'en')) }}
                            </option>
                        @endforeach
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-map me-1 text-primary"></i> {{ __('pos.governorate') }}
                    </label>
                    <select class="form-select @error('governorate_id') is-invalid @enderror"
                        wire:model="governorate_id">
                        <option value="">{{ __('pos.ph_governorate') }}</option>
                        @foreach ($governorates as $g)
                            <option value="{{ $g->id }}">
                                {{ $g->name ?? ($g->getTranslation('name', 'ar') ?? $g->getTranslation('name', 'en')) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-home-city-outline me-1 text-primary"></i> {{ __('pos.city') }}
                    </label>
                    <select class="form-select @error('city_id') is-invalid @enderror" wire:model="city_id"
                        {{ empty($governorate_id) ? 'disabled' : '' }}>
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-map-marker-outline me-1 text-primary"></i> {{ __('pos.area') }}
                    </label>
                    <select class="form-select @error('area_id') is-invalid @enderror" wire:model="area_id"
                        {{ empty($city_id) ? 'disabled' : '' }}>
                        <option value="">{{ __('pos.ph_area') }}</option>
                        @foreach ($areas as $a)
                            <option value="{{ $a->id }}">
                                {{ $a->name ?? ($a->getTranslation('name', 'ar') ?? $a->getTranslation('name', 'en')) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold">
                        <i class="mdi mdi-toggle-switch me-1 text-primary"></i> {{ __('pos.status') }}
                    </label>
                ...
                <div class="col-12 d-flex gap-2 mt-2">
                    <button class="btn btn-success rounded-pill px-4 shadow-sm">
                        <i class="mdi mdi-content-save-outline"></i> {{ __('pos.btn_update') }}
                    </button>
