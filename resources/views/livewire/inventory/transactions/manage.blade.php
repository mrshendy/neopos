<div class="container-fluid">
    ...
    @if ($errors->any())
        <div class="alert alert-danger alert-dismissible fade show shadow-sm mb-3">
            <i class="mdi mdi-alert-circle-outline me-2"></i><strong>حدثت أخطاء في الإدخال</strong>
    ...
    <div class="card shadow-sm rounded-4 stylish-card mb-4">
        <div class="card-header bg-light fw-bold">
            <i class="mdi mdi-swap-horizontal-bold me-1"></i> حركات المخزون
        </div>
        ...
        <div class="card-body">
            <form wire:submit.prevent="save" class="row g-3">
                ...
                <div class="col-lg-3">
                    <label class="form-label mb-1"><i class="mdi mdi-calendar"></i> تاريخ الحركة</label>
                ...
                <div class="col-lg-3">
                    <label class="form-label mb-1"><i class="mdi mdi-shuffle-variant"></i> نوع الحركة</label>
                ...
                <div class="col-lg-3">
                    <label class="form-label mb-1"><i class="mdi mdi-store-outline"></i> مخزن المصدر</label>
                    <select class="form-select" wire:model="warehouse_from_id" @disabled="!in_array($type,['out','transfer'])">
                ...
                <div class="col-lg-3">
                    <label class="form-label mb-1"><i class="mdi mdi-warehouse"></i> مخزن الوجهة</label>
                    <select class="form-select" wire:model="warehouse_to_id" @disabled="!in_array($type,['in','transfer','direct_add'])">
                        <option value="">— اختر —</option>
                        @foreach($warehouses as $w) <option value="{{ $w->id }}">{{ $w->name }}</option> @endforeach
                    </select>
                    <div class="preview-chip"><i class="mdi mdi-eye-outline"></i><span>
                ...
                <div class="col-12">
                    <label class="form-label mb-1"><i class="mdi mdi-note-text-outline"></i> ملاحظات</label>
                    <textarea class="form-control" rows="2" wire:model="notes" placeholder='{"ar":"ملاحظة","en":"Note"} أو نص عادي'></textarea>
                </div>

                <div class="col-12">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <div class="fw-bold"><i class="mdi mdi-format-list-bulleted"></i> تفاصيل الحركة</div>
                        <button type="button" class="btn btn-outline-primary rounded-pill px-3 shadow-sm" wire:click="addRow">
                            <i class="mdi mdi-plus"></i> إضافة صف
                        </button>
                    </div>
                        ...
                        <table class="table table-bordered align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:120px" class="text-center">الرصيد</th>
                                    <th style="width:120px">الكمية</th>
                                    <th style="width:160px">الوحدة (نص)</th>
                                    <th style="width:160px">تاريخ الصلاحية</th>
                                    <th style="width:160px">رقم التشغيلة</th>
                                    <th style="width:180px">السبب</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($rows as $i => $row)
                                    <tr>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-light btn-sm" onclick="confirmDelete({{ $i }})">
                                        ...
                                        <td>
                                            <input type="number" step="0.0001" class="form-control text-center" wire:model.lazy="rows.{{ $i }}.qty">
                                        ...
                                        <td>
                                            <input type="text" class="form-control" wire:model.lazy="rows.{{ $i }}.reason" placeholder="سبب الحركة (اختياري)">
    ...
    function confirmDelete(idx) {
        Swal.fire({
            title: 'تحذير',
            text: '⚠️ هل أنت متأكد من حذف الصف؟',
            icon: 'warning',
