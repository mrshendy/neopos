<div class="container-fluid">
    ...
    <div class="card shadow-sm rounded-4 stylish-card mb-4">
        <div class="card-header bg-light fw-bold d-flex align-items-center justify-content-between">
            <div><i class="mdi mdi-pencil-outline me-1"></i> {{ __('pos.trx_edit_title') }}</div>
            <div class="text-muted small">{{ __('pos.trx_no') }}: {{ $trx_no }}</div>
        </div>
        ...
        <div class="card-body">
            <form wire:submit.prevent="save" class="row g-3">
                ...
                <div class="col-lg-3">
                    <label class="form-label mb-1"><i class="mdi mdi-check-decagram-outline"></i> {{ __('pos.status') }}</label>
                            ...
                            <tbody>
                                @foreach($rows as $i => $row)
                                    <tr>
                                        ...
                                        <td><input type="text" class="form-control" wire:model.lazy="rows.{{ $i }}.uom" placeholder="PCS / BOX"></td>
                                        <td><input type="date" class="form-control" wire:model="rows.{{ $i }}.expiry_date"></td>
                                        <td><input type="text" class="form-control" wire:model.lazy="rows.{{ $i }}.batch_no" placeholder="BATCH-001"></td>
                                        <td><input type="text" class="form-control" wire:model.lazy="rows.{{ $i }}.reason" placeholder="{{ __('pos.reason_ph') }}"></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('inv.trx.index') }}" class="btn btn-outline-secondary rounded-pill px-4 shadow-sm">
    ...
    function confirmDelete(idx){
        ...
        }).then((r)=>{ if(r.isConfirmed){ Livewire.emit('deleteConfirmed', idx); Swal.fire('{{ __("pos.deleted") }}','{{ __("pos.row_deleted_ok") }}','success'); }});
    }
    </script>
</div>
