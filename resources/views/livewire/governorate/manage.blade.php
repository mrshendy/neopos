<div>
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">{{ trans('settings_trans.governorate') }}</h4>
        <button class="btn btn-primary" wire:click="openCreate">
            {{ trans('settings_trans.add_new_governorate') }}
        </button>
    </div>
    ...
    <div class="modal fade @if($showModal) show d-block @endif" tabindex="-1" style="@if($showModal)display:block;@endif">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ $editingId ? trans('settings_trans.update_governorate') : trans('settings_trans.add_new_governorate') }}</h5>
            ...
            <div class="modal-body">
                ...
                <div class="mb-2">
                    ...
                    <select class="form-select" wire:model.defer="id_country">
                        <option value="">{{ trans('settings_trans.country_select') }}</option>
                        @foreach($countries as $c)
                            <option value="{{ $c->id }}">{{ $c->name }}</option>
                        @endforeach
