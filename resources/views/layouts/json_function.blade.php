<script>
    $(document).ready(function() {
        $('select[name="id_country"]').on('change', function() {
            ...
            if (id_country) {
                $.ajax({
                    url: "{{ URL::to('get_governoratees') }}/" + id_country,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
                        $('select[name="id_governoratees"]').empty();
                        ...
                        $('select[name="id_governoratees"]').append('<option value="" selected disabled>{{ trans('patient_trans.select_option') }}</option>');
                        $.each(data, function(key, value) {
                            $('select[name="id_governoratees"]').append('<option value="' +
                            key + '">' + value + '</option>');
                        });
                    },
                });
    ...
    function get_city(id_governoratees)
     {
           var id_governoratees =id_governoratees;
            if (id_governoratees) {
                              $.ajax({
                                    url: "{{ URL::to('get_city') }}/" + id_governoratees,
                                    type: "GET",
                                    dataType: "json",
                                    success: function(data) {
                                        $('select[name="id_city"]').empty();
